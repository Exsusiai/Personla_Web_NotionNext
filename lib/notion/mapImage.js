...
const mapImgUrl = (img, block, type = 'block') => {
  ...
  if (!ret && (img.indexOf('prod-files-secure') > 0 || img.indexOf('secure.notion-static.com') > 0) && (BLOG.IMG_URL_TYPE === 'Notion' || type !== 'block')) {
    ret = 'https://www.notion.so/image/' + encodeURIComponent(img) + '?table=' + type + '&id=' + block.id
  }

  // const isNotionImg = (!ret && img.indexOf('secure.notion-static.com') > 0) || (!ret && img.indexOf('prod-files-secure') > 0)
