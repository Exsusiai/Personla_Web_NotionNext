...
async function getContact(post) {
  if (!post) {
    return null
  }

  post.blockMap = await getPostBlocks(post.id, 'data-contact')
  return post
}
...
async function getDataBaseInfoByNotionAPI({ pageId, from }) {
  ...
  const contact = await getContact(collectionData.filter(post => { return post && post?.type && post?.type === 'Contact' && post.status === 'Published' })?.[0])
